...
abstract class Either
{
     ...
     *
     * ```php
     * >>> Either::cond(true, fn() => 1, fn() => 'error');
     * => Right(1)
     *
     * >>> Either::cond(false, fn() => 1, fn() => 'error');
     * => Left('error')
     * ```
     *
     ...
     * @psalm-template LI
     * @psalm-template RI
     * @psalm-param (pure-callable) $left
     * @psalm-param (pure-callable) $right
     * @psalm-return Either<LI, RI>
     * @psalm-pure
     */
    public static function cond(bool $condition, callable $right, callable $left): Either
    {
        return $condition
            ? Right::of($right())
            : Left::of($left());
